 ...
 *	- made fftradix /fftradixf () scope, just use fftn()
...
void FFTN::transform1d(float *Buff, int N,  Bool Reverse, bool normalize)
{
   float *Pr = Buff;
   float *Pi = Buff + 1;
   int  Ndim = 1;
...
void FFTN::transform1d(double *Buff, int N,  Bool Reverse, bool normalize)
{
   double *Pr = Buff;
   double *Pi = Buff + 1;
   int  Ndim = 1;
...
void FFTN::transform2d(float *Buff, int Nx,  int Ny, Bool Reverse, bool normalize)
{
   float *Pr = Buff;
   float *Pi = Buff + 1;
   int  Ndim = 2;
...
void FFTN::transform2d(double *Buff, int Nx,  int Ny, Bool Reverse, bool normalize)
{
   double *Pr = Buff;
   double *Pi = Buff + 1;
   int  Ndim = 2;
...
void FFTN::transform3d(float *Buff, int Nx,  int Ny, int Nz, Bool Reverse, bool normalize)
{
   float *Pr = Buff;
   float *Pi = Buff + 1;
   int  Ndim = 3;
...
void FFTN::transform3d(double *Buff, int Nx,  int Ny, int Nz, Bool Reverse, bool normalize)
{
   double *Pr = Buff;
   double *Pi = Buff + 1;
   int  Ndim = 3;
...
#ifndef FFT_NOFLOAT
# define REAL		float
# define FFTNX		fftnf		/* trailing 'f' for float */
# define FFTNXS		"fftnf"		/* name for error message */
...
int FFTN::FFTNX (int ndim, const int dims[],
      REAL Re [],
      REAL Im [],
      int iSign,
      double scaling)
{
	...
	for (i = 0; i < ndim; i++)
	  {
	     if (dims [i] <= 0)
	       {
		  fputs ("Error: " FFTNXS "() - dimension error\n", stderr);
		  return -1;
	       }
	     nTotal *= dims [i];
	  }
   ...
   for (i = 0; i < ndim; i++)
     {
	nSpan = dims [i];
	if (nSpan > (size_t) max_factors) max_factors = nSpan;
	if (nSpan > (size_t) max_perm) max_perm = nSpan;
     }
#else
	  ...
	  int max_perm)
{
   ...
   if (MaxPermAlloced < (size_t) max_perm)
     {
#ifdef SUN_BROKEN_REALLOC
