...
// cout << s+1 << " Alpha = " << Alpha << ", Nsig = " << NSigma[s] << ", PDet = " << PDet << endl;
	       for (i = 0; i < Np; i++)
 	       {
	          if (TabPValue(i) < PDet) 
		  {
...
// cout << s+1 << " Alpha = " << Alpha << ", Nsig = " <<(double) NSigma[s] << endl;
// printf("%f\n", PDet);
	    }
	    break;
...
cout << "set_sigma : Noise = " << SigmaNoise << endl;
 ...
 cout << "After set_sigma : Noise = " << SigmaNoise << endl;
	...
	if  ((Type_Transform == TO1_PAVE_B3SPLINE) || (Type_Transform == TU1_MALLAT))
	{
