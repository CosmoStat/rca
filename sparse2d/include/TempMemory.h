...
#define MEM_NOT_MANADGE 1

#undef DEBUG_MEM
#define DEBUG_MEM 0

#define MAX_IMA_IN_MEM 10000
...
#if MEM_NOT_MANADGE            
   return (PARAM_TYPE*)(alloc_buffer((size_t) (Nelem*sizeof(PARAM_TYPE))));
#else
   int i=0;
   Bool Find=False;
   PARAM_TYPE* Pf=NULL;

   //while ((TabBuffMem[i].size() != 0) && (!Find) && (i < MAX_IMA_IN_MEM))
   while ((TabBuffMem[i].size() != 0) && (!Find))
	{
...
#if DEBUG_MEM
		cout << "Alloc Find: " << i << "  Ptr = " << Pf << endl;
#endif
      return Pf;
   }
	//else if (i < MAX_IMA_IN_MEM)
	else 
	{
...
//   while (   (TabBuffMem[i].size() != 0) && (!Find)) {
   ...
   if (Find) TabBuffMem[i].give_back();
      ...
      cerr << "Error: CMemInt cannot deallocate the memory ... " << endl;
      exit (0);
   }
#endif
};
